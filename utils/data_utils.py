...
class OfficeDataset(Dataset):
    def __init__(self, base_path, site, train=True, transform=None):
        if train:
            self.paths, self.text_labels = np.load('/media/ihpc/wangmeng/FL_CVPR/DataImbalance/FedBNUncertaintyKD/data/office_caltech_10/{}_train.pkl'.format(site), allow_pickle=True)
        else:
            self.paths, self.text_labels = np.load('/media/ihpc/wangmeng/FL_CVPR/DataImbalance/FedBNUncertaintyKD/data/office_caltech_10/{}_test.pkl'.format(site), allow_pickle=True)
...
class DomainNetDataset(Dataset):
    def __init__(self, base_path, site, train=True, transform=None):
        if train:
            self.paths, self.text_labels = np.load('/media/ihpc/wangmeng/FL_CVPR/DataImbalance/FedBNUncertaintyDis/data/DomainNet/{}_train.pkl'.format(site), allow_pickle=True)
        else:
            self.paths, self.text_labels = np.load('/media/ihpc/wangmeng/FL_CVPR/DataImbalance/FedBNUncertaintyDis/data/DomainNet/{}_test.pkl'.format(site), allow_pickle=True)
